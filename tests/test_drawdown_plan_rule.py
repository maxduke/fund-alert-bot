...
def test_sma_values_are_unavailable_with_insufficient_history() -> None:
    ...
    assert calculate_sma(history, window=3) == pytest.approx(3)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda: calculate_sma(_history([1, 2]), 2.5), "window"),
        (lambda: calculate_sma_slope(_history([1, 2]), 2, 0), "slope_window"),
        (lambda: calculate_sma_slope(_history([1, 2]), 2, True), "slope_window"),
