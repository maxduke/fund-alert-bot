...
首次实跑正常后，使用现有命令和日志做日常观察。时间以部署的 `TZ` 和实际调度
配置为准；默认盘前收盘检查为 `14:50`、收盘检查为 `17:10`、次日净值任务为
`08:30`，DCA 提醒为 `09:30`。不要求为了验收临时修改规则或制造真实加仓记录。

| 场景 | 检查方式 | 正常结果 |
| --- | --- | --- |
| 完整调度周期 | 各任务运行后查看 `/status`，对照收到的提醒与 `/plans` | 任务记录持续更新；提醒标明正确的数据日期；无符合条件的规则时可以没有提醒 |
| 提醒去重 | 对照同一规则、周期、档位和市场日期 | 同日不重复创建提醒；未处理档位可按规则在后续日期再次提醒，不视为重复故障 |
| 净值延迟或数据源失败 | 查看数据日期、任务结果和待估算数量 | 保持待处理，不使用过期净值入账；目标日期净值可用后只应用一次 |
| 节假日与跨日 | 对照增强 DCA 的 `holiday:next` 或 `holiday:skip` 配置 | 顺延保留原期次身份；跳过不估算持仓；不补建规则创建前的期次 |
| 持仓对账 | 对照 `/plans`、实际平台记录和此前同步结果 | 估算明确标记；每笔 DCA/手动加仓只计入一次；同步后不重复累计 |
| 常规重启或升级后 | 查看 `/status`、已有档位及待处理项的后续结果 | 档位和持仓状态保留；待处理工作恢复；恢复投递不重复生成期次或持仓估算 |

优先用 `/status` 读取本地运行记录，按需用 `/plans` 查看业务状态。`/check` 会
评估普通回撤、固定成本涨幅和到期 DCA，可能发送正式提醒，因此不作为无副作用
的日常状态探针。缓存日期较新也不代表所有规则都成功检查。

待投递、失败投递或待估算数量短时增加不一定是故障；结合原因和下一次相关任务
判断是否恢复。目标日期净值已可用却持续不入账、多个相关运行后仍无恢复、任务
记录未按配置时间更新，或出现重复累计时，记录规则/期次、源数据日期与任务结果，
作为单独问题排查。无需为了覆盖异常场景主动中断生产服务。

观察记录保存在私有运维笔记中，可使用以下格式；未发生的边界场景保持待观察，
不能仅凭“实跑正常”标记为已验证：
...
首次完整调度周期观察完成后转为日常关注异常；节假日、净值延迟和重启随实际
发生补记。备份与恢复演练继续按下方“备份 SQLite”执行。
